...
function clearData(){
    ...
    dataPlot1.data.labels=[];
    dataPlot1.data.datasets[0].data=[];
    dataPlot1.update();
    records = new Map();    
}
...
function uiupdata(idata,udata){
    ...
    dataPlot1.data.labels=keys;
    dataPlot1.data.datasets[0].data=values;

    
    end=keys.length;
    var result = linearRegression(keys, values);

    for (i=0;i<end;i++){
        dataPlot1.data.datasets[1].data[i]=parseFloat(result.intercept)+parseFloat(keys[i]-keys[0])*result.slope;
    };
...
function linearRegression(x, y) {
    // 计算平均值
    let xMean = x.reduce((a, b) => a + parseFloat(b), 0) / x.length;
    let yMean = y.reduce((a, b) => a + parseFloat(b), 0) / y.length;
    // 计算斜率和截距
    let numerator = 0;
    let denominator = 0;
    ...
    let slope = numerator / denominator;//斜率
    let intercept = yMean - slope * xMean;//截距
    slope=parseFloat(slope).toFixed(3);
    intercept=parseFloat(intercept).toFixed(3);
  

    document.getElementById("label4").textContent="趋势线:"+"U="+slope+"*I"+"+"+intercept;
    // 返回拟合结果
    return {
      slope: slope,
      intercept: intercept
